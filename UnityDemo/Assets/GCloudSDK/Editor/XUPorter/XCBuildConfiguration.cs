...
namespace GCloudSDKEditor.XCodeEditor
{
	public class XCBuildConfiguration : PBXObject
	{
		...
		public bool overwriteBuildSetting (string settingName, PBXList flags)
		{
			bool modified = false;
			...
			foreach (string flag in flags) {
				...
				if (!((PBXList)((PBXDictionary)_data [BUILDSETTINGS_KEY]) [settingName]).Contains (flag)) {
					((PBXList)((PBXDictionary)_data [BUILDSETTINGS_KEY]) [settingName]).Add (flag);
					modified = true;
				}
			}
