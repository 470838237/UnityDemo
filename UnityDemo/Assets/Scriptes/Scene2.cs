...
public class Scene2 : BaseScene
{
    ...
    private void GetServerListSuccess(ServerList result)
    {
        ...
        Debug.Log("HonorSDK:options[0].text="+ options[0].text);
        Debug.Log("HonorSDK:captionText.text=" + optionView.itemText.text);
        optionView.itemText.text = options[0].text;
        ...
        optionView.onValueChanged.AddListener(delegate(int index)
        {
            ...
            optionView.itemText.text = result.servers[index].serverName;
            Debug.Log("HonorSDK:options[0].text=" + options[index].text);
        });
