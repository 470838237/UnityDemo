...
namespace GCloudSDKEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		public bool overwriteBuildSetting (string settingName, ArrayList arrayListValue, string buildConfigName = "all")
		{
			Debug.Log ("overwriteBuildSetting " + settingName + " " + arrayListValue + " " + buildConfigName);
			foreach (KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations) {
				//Debug.Log ("build config " + buildConfig);
				XCBuildConfiguration b = buildConfig.Value;
				if ((string)b.data ["name"] == buildConfigName || (string)buildConfigName == "all") {
					//Debug.Log ("found " + b.data["name"] + " config");
					buildConfig.Value.overwriteBuildSetting (settingName, arrayListValue);
					modified = true;
				} else {
					//Debug.LogWarning ("skipping " + buildConfigName + " config " + (string)b.data["name"]);
				}
		...
		public PBXDictionary AddFile (string filePath, PBXGroup parent = null, string tree = "SOURCE_ROOT", bool createBuildFiles = true, bool weak = false)
		{
			...
			if (fileReference != null) {
				...
				if (!filePath.EndsWith("CoreLocation.framework")) {//fix corelocation lack for some unity version
					return null;
				}
			}
		...
		public bool AddFolder (string folderPath, PBXGroup parent = null, string[] exclude = null, bool recursive = true, bool createBuildFile = true)
		{
			...
			#if UNITY_5
			if (folderPath.Contains ("C98")) {
				folderPath = folderPath.Replace ("C98", "C11");
			}
			#else
			if(folderPath.Contains("C11")){
			folderPath = folderPath.Replace("C11","C98");
			}
			#endif
			...
			foreach (string directory in Directory.GetDirectories( folderPath )) {
				...
				if (directory.EndsWith(".framework") ||
					...
					directory.EndsWith(".h") ||
					directory.EndsWith(".m") ||
					directory.EndsWith(".mm")||
					directory.EndsWith(".bundle")) {

					this.AddFile (directory, newGroup);
					continue;
				}
				if (recursive) {
					Debug.Log ("recursive");
		...
		public void transformJSONToPlist (DictionaryEntry entry)
		{
			if (entry.Value is string) {
				...
				Debug.Log ("Ttransform JSON To Plist value is unexpected!");
			}
		...
		{
			Debug.Log ("CreateNewProject PATH: " + path);
			PBXParser parser = new PBXParser ();
